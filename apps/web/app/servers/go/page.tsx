...
export default function GoServerPage() {
  const { t } = useLanguage()
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          <h1 className="text-4xl font-bold mb-2">Go OAuth2 Server</h1>
          <p className="text-gray-600">Gin + go-oauth2/oauth2</p>
        </div>
        <span className={`px-4 py-2 rounded-lg text-sm font-medium ${status === 'running' ? 'bg-green-100 text-green-800' : status === 'stopped' ? 'bg-gray-100 text-gray-800' : 'bg-yellow-100 text-yellow-800'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="border rounded-lg p-6">
          ...
          <dl className="space-y-2">
            <div className="flex justify-between"><dt className="text-gray-600">{t.servers.port}:</dt><dd className="font-medium">9000</dd></div>
            <div className="flex justify-between"><dt className="text-gray-600">{t.servers.framework}:</dt><dd className="font-medium">Gin</dd></div>
            <div className="flex justify-between"><dt className="text-gray-600">{t.servers.oauthLibrary}:</dt><dd className="font-medium">go-oauth2/oauth2</dd></div>
        ...
        <div className="border rounded-lg p-6">
          ...
          <div className="space-y-3">
            ...
            <a href="http://localhost:9000/health" target="_blank" rel="noopener noreferrer" className="block w-full px-4 py-2 bg-gray-100 text-center rounded hover:bg-gray-200">{t.servers.openHealth}</a>
          </div>
        </div>
      </div>
      <div className="border rounded-lg p-6 mb-8">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center p-3 bg-gray-50 rounded">
            ...
            <span className="text-gray-600">{t.servers.authEndpoint}</span>
          </div>
          <div className="flex justify-between items-center p-3 bg-gray-50 rounded">
            <span className="font-mono text-sm">POST /oauth/token</span>
            <span className="text-gray-600">{t.servers.tokenEndpoint}</span>
          </div>
        </div>
      </div>
